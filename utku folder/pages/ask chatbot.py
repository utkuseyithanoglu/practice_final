...
@st.cache_resource
def load_models():
    ROOT = Path(__file__).resolve().parents[2]   # pages -> utku folder -> folder (root)
...
def load_all_data():
    ...
    hourly_df  = pd.read_csv(DATA / "hourly_location_avg.csv")
    monthly_df = pd.read_csv(DATA / "monthly_location_avg.csv")
    rf_preds   = pd.read_csv(DATA / "hourly_rf_predictions.csv")

    # bu dosya raw'da değilse yolunu ona göre ayarla:
    df2 = pd.read_csv(ROOT / "utku folder" / "ALL_BOROUGHS_SARIMA_72H.csv")
